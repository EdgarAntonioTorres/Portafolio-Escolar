# -*- coding: utf-8 -*-
"""
Created on Thu Sep 26 18:09:05 2024
...
class SimonDice:
    def __init__(self, ventana):
        #Incializar la ventana y las variables del juego
        ...
        #Ronda actual
        self.ronda = 1
        #Índice para controlar la recursión en la reproducción de la secuencia
    ...
    def crear_interfaz(self):
        ...
        self.boton_mostrar = tk.Button(self.ventana, bg = 'grey', width=20, height=5, state='disable')
    ...
    def mostrar_secuencia(self):
        ...
        #Añadir un nuevo color aleatorio a la secuncia del juego
        ...
        #Desactivar el boton de empezar
        self.boton_empezar.config(state = 'disabled')
        #Iniciar la reproducción de la secuencia desde el primer índice
        self.indice_secuencia = 0
        self.reproducir_secuencia()
        
    def reproducir_secuencia(self):
        #Si aún hay colores en la secuencia por mostrar
    ...
    def apagar_color(self):
        #Apagar el color (Volver el botón a color gris)
        self.boton_mostrar.config(bg = 'grey')
    ...
    def boton_clic(self, nombre_color):
        ...
        if self.esperando_input:
            ...
            #Verificar si el último color ingresado es correcto
            if self.secuencia_jugador[indice_actual] != self.secuencia_juego[indice_actual]:
                ...
                tk.messagebox.showerror("Error", "¡Incorrecto! Hasta aquí llegaste JAJA")
                self.reiniciar_juego()
            elif len(self.secuencia_jugador) == len(self.secuencia_juego):
                #Si la secuencia está conpketa y es correcta, avanzar a la siguiente ronda
                self.ronda += 1
                tk.messagebox.showinfo("CORRECTO", "¡Increíble! Eres una bestia")
                #Esperar un segundo antes de inciar la siguiente ronda
